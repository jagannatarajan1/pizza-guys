...
const CONFIG: Record<OtpScope, ScopeConfig> = {
  ...
  // lower and a wrong code is far more expensive. Attempt budget stays tighter
  // than the customer door even though the resend cooldown is the same 30s.
  admin: {
    maxAttempts:     3,
    cooldownSeconds: 30,
...
// Bare `{ ok: false }` (no reason) is what a passwordless-door caller should
// surface to the browser, to keep "wrong code" and "unknown address"
// indistinguishable. A caller downstream of a password check — where the
// enumeration threat this protects against no longer applies, since the
// caller already proved they know the account's password — may read `reason`
// to show a more specific, still safe, message.
export type RedeemFailReason =
  | 'no_code' | 'wrong_scope' | 'expired' | 'attempts_exhausted'
  | 'not_eligible' | 'locked' | 'incorrect' | 'already_used' | 'unknown_account'

export type RedeemResult =
  | { ok: true; user: RedeemedUser }
  | { ok: false; reason: RedeemFailReason }

// Safe, specific copy for a caller downstream of a password check (see the
// RedeemFailReason doc comment above for why specificity is fine here).
// Deliberately collapses every "this shouldn't normally happen" reason to the
// same generic line as a wrong code, rather than a bespoke message per case.
export function describeRedeemFailure(reason: RedeemFailReason): string {
  switch (reason) {
    case 'expired':            return 'This verification code has expired. Please request a new code.'
    case 'attempts_exhausted': return 'Too many incorrect attempts. Please request a new verification code.'
    case 'already_used':       return 'This verification code has already been used. Please request a new code.'
    default:                   return 'Invalid verification code. Please try again.'
  }
}
...
export async function redeemLoginOtp(params: {
  ...
  const fail = async (detail: string, reason: RedeemFailReason): Promise<RedeemResult> => {
    await logAuditEvent({ userId: user.id, email: user.email, action: 'login_otp_failed', detail: `scope=${scope} ${detail}`, ip })
    return { ok: false, reason }
  }

  if (!user.loginOtpHash || !user.loginOtpExpires) return fail('no code outstanding', 'no_code')
  ...
  if (user.loginOtpScope !== scope) return fail('wrong scope', 'wrong_scope')
  ...
  if (user.loginOtpAttempts >= maxAttempts) {
    await burnCode(user.id, user.loginOtpHash)
    return fail('attempts exhausted', 'attempts_exhausted')
  }
  ...
  if (scope === 'admin' && !STAFF_ROLES.includes(user.role)) {
    await burnCode(user.id, user.loginOtpHash)
    return fail('not eligible for staff login', 'not_eligible')
  }
  if (user.lockedUntil && user.lockedUntil > new Date()) return fail('account locked', 'locked')
  ...
  if (!verifyOtp(code, user.loginOtpHash)) {
    ...
    const justExhausted = user.loginOtpAttempts + 1 >= maxAttempts
    await prisma.user.updateMany({
      where: { id: user.id, loginOtpHash: user.loginOtpHash, loginOtpAttempts: { gte: maxAttempts } },
      data:  { loginOtpHash: null, loginOtpExpires: null, loginOtpAttempts: 0, loginOtpScope: null },
    })
    // The guess that pushes the counter to the limit destroys the code right
    // here — tell the caller that outright rather than "wrong code, try
    // again" when trying again is no longer possible.
    return justExhausted ? fail('incorrect code — attempts exhausted', 'attempts_exhausted') : fail('incorrect code', 'incorrect')
  }
  ...
  if (!claimed) return fail('code already used', 'already_used')
